...
public class Graph extends JPanel {
    ...
    // the gap between window border and
    ...
    public Graph(ArrayList<Double> dataSet) {
    ...
    public void paintComponent(Graphics g) {
        ...
        /*
         * creating scale for the graph
         * 
         * used for being referance when drawing lines
         */
        double xScale = ((double) getWidth() - 2 * BORDER_GAP) / (dataSet.size() - 1);

        /*
         * using dataSet and getting location of points
         */
        List<Point> graphPoints = new ArrayList<Point>();
        List<Double> graphPointsPrice = new ArrayList<Double>();
        for (int i = 0; i < dataSet.size(); i++) {
            int x1 = (int) (i * xScale + BORDER_GAP);
            int y1 = (int) (Math.round(this.getYLocation(dataSet.get(i))) + BORDER_GAP);
        ...
        for (int i = 0; i < Y_HATCH_CNT; i++) {
            ...
            g2.drawString("$" + String.valueOf(getHashLabel(i)), x0 + 10, y0 + 5);
        }
        ...
        for (int i = 0; i < dataSet.size() - 1; i++) {
            int x0 = (i + 1) * (getWidth() - BORDER_GAP * 2) / (dataSet.size() - 1) + BORDER_GAP;
            int x1 = x0;
    ...
    private int getYLocation(Double price) {
        int graphableHeight = this.getHeight() - (BORDER_GAP * 2); // calculates pixels of graph
